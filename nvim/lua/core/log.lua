...
local is_override_notify = false
...
function Log:set_level(level)
  local logger_ok, _ = xpcall(function()
    ...
    if structlog then
      local logger = structlog.get_logger "mylog"
  ...
  local packer_ok, _ = xpcall(function()
    ...
    require("packer.log").new { level = current_log_level }
  end, debug.traceback)
...
function Log:init()
  ...
  local log_level = Log.levels[(current_log_level):upper() or "WARN"]
  local config = {
    mylog = {
      sinks = {
...
function Log:get_path()
  return string.format("%s/%s.log", vim.call("stdpath", "cache"), "mylog")
end
