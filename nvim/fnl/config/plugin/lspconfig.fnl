...
(tset (require :lspconfig.configs) :fennel_language_server
      {:default_config {:cmd [(vim.fn.expand :$HOME/.cargo/bin/fennel-language-server)]
                        :filetypes [:fennel]
...
(let [handlers {:textDocument/publishDiagnostics (vim.lsp.with vim.lsp.diagnostic.on_publish_diagnostics
  ...
  (lsp.fennel_language_server.setup {: on_attach : handlers : capabilities})
  ;; go
  ...
  ;; html / css / json
  (lsp.cssls.setup {: on_attach
                    : handlers
                    : capabilities
                    :cmd [:vscode-css-languageserver :--stdio]})
  (lsp.html.setup {: on_attach
                   : handlers
                   : capabilities
                   :cmd [:vscode-html-languageserver :--stdio]})
  (lsp.jsonls.setup {: on_attach
                     : handlers
                     : capabilities
                     :cmd [:vscode-json-languageserver :--stdio]}))

(tset (require :vim.lsp.protocol) :CompletionItemKind
      ["   (Text) "
       "   (Method)"
       "   (Function)"
       "   (Constructor)"
